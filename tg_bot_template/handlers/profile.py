...
@router.message(
    UserForm.name, F.text.strip() != "", lambda message: len(message.text.strip()) <= 30
)
...
@router.message(
    ...
    F.text,
    F.text.strip() != "",
    lambda message: len(message.text.strip()) <= 100,
)
async def process_info(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Отлично, записал. Теперь скиньте свое фото.", reply_markup=cancel_keyboard()
    )
...
async def process_photo(message: Message, state: FSMContext, session: AsyncSession):
    ...
    await message.answer("Готово, данные обновлены.", reply_markup=main_keyboard())
