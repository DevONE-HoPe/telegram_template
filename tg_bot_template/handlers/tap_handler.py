...
from tg_bot_template.keyboards.inline.click import (
    GameButtonCallback,
...
async def handle_game_tap(
    query: CallbackQuery, callback_data: GameButtonCallback, session: AsyncSession
) -> None:
    ...
    try:
        await query.message.edit_text(text=text, reply_markup=new_keyboard)
    except TelegramBadRequest:
        await query.message.answer(text=text, reply_markup=new_keyboard)
